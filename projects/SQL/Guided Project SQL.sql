...
--Database Size
SELECT pg_database_size('AlmaX') /*Shows Value in Bytes */
Select pg_size_pretty(pg_database_size('AlmaX')) /*Shows Value in Kilo/MegaBytes..,based on how big the number is*/

--Table Size
Select pg_relation_size('Store')
...
/* Get Column name for the Data*/
Select Column_name
from information_schema.columns
where table_name='store';

/*Get Column name with datatype*/
...
/*Dropping Unneccessary Columns like Row_id*/
Alter Table Store
Drop Column row_id;
...
Select count(distinct(product_name)) as Unique_product_Counts from store 

/*Which are the top 10 products ordered frequently*/
Select product_name,Count(*) as num_of_orders
From store
group by product_name
order by 2 desc
limit 10;

/*Calculate the cost for each order_id with respective order_name*/
Select order_id,product_name,Round(CAST((sales-profit) as Numeric),2) as cost_of_order from Store;
	--Output will be in double precision need to cast it as numeric to round off

/* Calculate % profit for each order with respective Product_name*/
...
from store as a
left join
(Select product_name,order_id,ROUND(CAST((profit/((sales-profit))*100)AS NUMERIC),2) as percentage_profit  
from store
group by order_id,product_name,percentage_profit) as b
...
/* Where can we trim some loses? 
 ...
 CREATE TEMPORARY TABLE t0 as
 Select ROUND(CAST(AVG(profit)as NUMERIC),2) as average_profit,ROUND(CAST(AVG(sales)as NUMERIC),2)as average_sales 
 from store;
 CREATE TEMPORARY TABLE t1 as
 SELECT sub_category,ROUND(CAST(SUM(Sales)as NUMERIC),2)as Sales,ROUND(CAST(SUM(profit)as NUMERIC),2)as profit from store
 group by sub_category
...
where order_date BETWEEN('2014-01-01')and('2016-12-31')
